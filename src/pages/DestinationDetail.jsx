      ...
      <section className="py-24 dark:bg-gray-900">
        <div className="max-w-[1280px] mx-auto px-5">
          <div className="grid grid-cols-[1fr_380px] gap-10 items-start max-xl:grid-cols-1">
            {/* Main */}
            <div>
              <div className="grid grid-cols-4 gap-4 p-6 bg-white dark:bg-gray-800 rounded-2xl shadow-glass-lg border border-gray-100 dark:border-gray-700 mb-10 max-md:grid-cols-2 max-sm:grid-cols-1" data-aos="fade-up">
                {[
                  ...
                  <div key={i} className="flex items-center gap-3">
                    ...
                    <div>
                      <span className="block text-[0.72rem] text-gray dark:text-gray-400 uppercase tracking-wider">{label}</span>
              ...
              <div className="mb-10" data-aos="fade-up">
                ...
                <p className="text-gray leading-relaxed dark:text-gray-400">{dest.description}</p>
              </div>
              ...
              <div className="mb-10" data-aos="fade-up">
                ...
                <div className="pl-8 relative before:absolute before:left-2 before:top-0 before:bottom-0 before:w-0.5 before:bg-gray-100 dark:before:bg-gray-700">
                  {dest.itinerary.map((day, i) => (
                    <div key={i} className="relative pb-7 last:pb-0">
                      <div className="absolute left-[-28px] top-1 w-[18px] h-[18px] rounded-full bg-white dark:bg-gray-900 border-[3px] border-primary" />
                      ...
                      <p className="text-gray text-sm leading-relaxed dark:text-gray-400">{day.description}</p>
                    </div>
                  ))}
                </div>
              </div>
                ...
                <div>
                  ...
                  <ul className="flex flex-col gap-3">
                    {dest.included.map((item, i) => (
                      <li key={i} className="flex items-center gap-2.5 text-sm text-gray dark:text-gray-400"><FiCheck className="text-primary shrink-0" /> {item}</li>
                    ))}
                  </ul>
                </div>
                <div>
                  ...
                  <ul className="flex flex-col gap-3">
                    {dest.excluded.map((item, i) => (
                      <li key={i} className="flex items-center gap-2.5 text-sm text-gray dark:text-gray-400"><FiX className="text-danger shrink-0" /> {item}</li>
                    ))}
                  </ul>
                </div>
              </div>
            ...
            <div className="xl:sticky xl:top-24" data-aos="fade-left">
              <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-glass-xl overflow-hidden border border-gray-100 dark:border-gray-700">
                <div className="text-center py-7 bg-gradient-to-br from-navy to-navy-light text-white">
                  <h2 className="text-accent text-xl font-heading">{dest.name}</h2>
                  <span className="text-sm opacity-60">{dest.location}</span>
                </div>
                <div className="p-6 flex flex-col gap-3.5 border-b border-gray-100 dark:border-gray-700">
                  {[['Duration', dest.duration], ['Difficulty', dest.difficulty], ['Altitude', dest.altitude], ['Best Time', dest.bestTime]].map(([label, value]) => (
                    <div key={label} className="flex justify-between items-center">
                      <span className="text-gray text-sm dark:text-gray-400">{label}</span>
